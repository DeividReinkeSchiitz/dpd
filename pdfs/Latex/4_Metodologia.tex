...
O PDD é formulado como um programa linear inteiro binário, instanciando a estrutura geral de PLI apresentada na Equação~\eqref{eq:pli_geral}. A notação adotada é definida a seguir, seguida da formulação completa do modelo.

\subsection{Notação}

A Tabela~\ref{tab:notacao} reúne os conjuntos, parâmetros e variável de decisão do modelo.
...
\hline
$P = \{1,\ldots,|P|\}$ & Conjunto de professores, indexado por $i$ \\
$C = \{1,\ldots,|C|\}$ & Conjunto de disciplinas, indexado por $j$ \\
...
A função objetivo~\eqref{eq:func_obj} maximiza a soma ponderada das atribuições: valores positivos de $w_{i,j}$ refletem compatibilidade de área e preferência declarada, enquanto o valor negativo $-\text{area\_penalty}$ penaliza atribuições fora das áreas de especialização do professor.

A restrição~\eqref{eq:cobertura} garante que cada disciplina seja atribuída a exatamente um professor. As restrições~\eqref{eq:ch_min_anual} asseguram que cada professor cumpra a carga horária mínima anual. As restrições~\eqref{eq:ch_max_sem1} e~\eqref{eq:ch_max_sem2} limitam, respectivamente, a carga semestral no primeiro e no segundo semestres. O domínio~\eqref{eq:var_decisao} restringe as variáveis ao conjunto binário.
