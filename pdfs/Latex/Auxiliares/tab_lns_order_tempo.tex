% ============================================
% TABELA 4: Impacto da Ordenação - Tempo
% ============================================
\begin{table}[htbp]
\centering
\caption{Tempo de execução (segundos) comparando ordenação crescente e decrescente. Cenário avaliado: 75\% de destruição, tempo LNS = 30s, limite global = 400s.}
\label{tab:lns_order_tempo}
\small
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
\textbf{Instância} & \textbf{Disciplinas} & \textbf{Profs} & \textbf{B\&B} & \textbf{Crescente} & \textbf{Decrescente} & \textbf{Ajuste} \\
\midrule
input4  & 123 & 46 & 400,05 & 400,04 & 400,05 & Crescente \\
input5  & 214 & 69 & 114,18 & 140,64 & 141,47 & Crescente \\
input12 & 115 & 37 & 5,79   & 25,70  & 25,84  & Crescente \\
input15 & 94  & 30 & 400,03 & 400,02 & 400,02 & Empate \\
input16 & 89  & 28 & 3,07   & 0,72   & 0,72   & Empate \\
input17 & 108 & 38 & 400,04 & 400,04 & 400,03 & Decrescente \\
input20 & 104 & 37 & 0,90   & 4,40   & 4,32   & Decrescente \\
input33 & 104 & 36 & 400,03 & 400,03 & 400,03 & Empate \\
input37 & 152 & 53 & 68,00  & 31,61  & 31,62  & Crescente \\
input38 & 62  & 22 & 0,16   & 0,28   & 0,28   & Empate \\
\midrule
% \textbf{Média}  & 106 & 37 & 91,09 & 86,12 & 86,55 & Crescente \\
\textbf{Total}  & 1.165 & 396 & 1.792,24 & \textbf{1.803,48} & 1.804,48 & \textbf{Crescente} \\
\bottomrule
\end{tabular}
\end{table}
