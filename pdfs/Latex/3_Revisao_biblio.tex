...
Na literatura, distinguem-se duas variantes principais do esquema. No \textit{fix-and-relax}, as variáveis fixadas são removidas do subproblema e as variáveis liberadas permanecem inteiras, gerando um sub-MIP de dimensão estritamente menor que o problema original. No \textit{fix-and-optimize} (também denominado \textit{local branching} \cite{pisinger_ropke_2010}), as variáveis fixadas são mantidas no modelo como restrições de igualdade, preservando a estrutura completa do original, o que pode facilitar a aplicação de cortes e planos seccionais do solver. Para o PDD, o esquema \textit{fix-and-relax} é preferido porque gera subproblemas de dimensão reduzida --- apenas as variáveis $x_{i,j}$ dos candidatos selecionados permanecem livres ---, tornando o sub-MIP mais tratável sem a necessidade de acrescentar restrições extras ao modelo.
