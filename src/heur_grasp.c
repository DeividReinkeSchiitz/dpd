...
{
  const Course *turmaA = (const Course *) a;
  const Course *turmaB = (const Course *) b;

  // usando o ponteiro global g_covered para saber se a turma esta coberta
  int coveredA         = g_covered ? g_covered[turmaA->number - 1] : 0;
  int coveredB         = g_covered ? g_covered[turmaB->number - 1] : 0;
  ...
  if (turmaA->level < turmaB->level) return -1;
  if (turmaA->level > turmaB->level) return 1;
  return 0;
}
...
void reset_class(Course *turmas, int m)
{
...
void adaptive_edges(Professor *professores, Course *turmas, int *covered, int n, int m)
{
  ...
  for (int p = 0; p < n; p++)
  {
    ...
    if (check_ch(professores[p].current_CH1, professores[p].current_CH2, professores[p].minWorkload) == 1) continue;
    nscore = 0;
    ...
    for (int t = 0; t < m; t++)
    {
      ...
      if (turmas[t].semester == 1 && professores[p].current_CH1 + turmas[t].workload > professores[p].maxWorkload1) continue;
      if (turmas[t].semester == 2 && professores[p].current_CH2 + turmas[t].workload > professores[p].maxWorkload2) continue;

      // verificado se a turma j foi escolhida pelo prof i
      pref = check_preference(professores[p].preferences, turmas[t].label, m);
      if (pref != 0)
      {
        ...
        professores[p].scores[nscore].score        = score;
        professores[p].scores[nscore].codigo_turma = t + 1;  // salvando a turma no prof
        nscore++;
        turmas[t].level++;  // incrementando o n de profs que podem ministrar a turma

        // nao foi escolhida. verificando se a turma j é da area do prof i
      }
      else if (checaArea(professores[p].areas, turmas[t].subject.areas) == 1)
      {
        //printf("PROF %s pode ministrar a turma %d\n", professores[i].nome, turmas[j].codigo);
        alfa                                       = random_number(1, 8);
        //score = calculaScore(0, alfa);  // score quando n tem pref eh so o alfa. posso tentar mudar isso depois
        professores[p].scores[nscore].score        = alfa;
        professores[p].scores[nscore].codigo_turma = t + 1;  // salvando a turma no prof
        nscore++;
        turmas[t].level++;
      }
    }
    professores[p].level = nscore;  // salvando a quant de turmas que sao da area do prof p
  }
}
        ...
        float alpha)
{
  ...
  while (*nCovered < m)
  {
    // Course turma = turmas[0]; // pegando sempre as insfo da primeira turma (ordenada)
    ...
    for (t = 0; t < m; t++)
    {
      ...
      for (int p = 0; p < n; p++)
      {
        ...
        if (professores[p].current_CH1 + professores[p].current_CH2 >= professores[p].minWorkload) continue;

        // indo na lista de pref do prof p, e verificando se a turma t esta la
        for (int s = 0; s < professores[p].level && professores[p].scores[s].codigo_turma <= turma.label; s++)
        {

          // printf("turma: %d ; turma escolhida pelo prof: %s\n",professores[p].scores[s].codigo_turma ,professores[p].nome);
          // verificando se o prof p tem a materia t na sua lista
          if (turma.label == professores[p].scores[s].codigo_turma)
          {
            ...
            if (turma.semester == 1)
            {
              ...
              if (professores[p].current_CH1 + turma.workload <= professores[p].maxWorkload1)
              {
                candidate_scores[num_candidates].codigo_turma = p;  // p = 0 --> amaury
                candidate_scores[num_candidates].score        = professores[p].scores[s].score;
                candidate_vars[num_candidates]                = varlist[p * m + turma.label - 1];
                num_candidates++;

                //printf("\nVARIAVEL SELECIONADA: %s\n", SCIPvarGetName(varlist[p*m + turma.label-1]));
              }
            }
            else
            {
              // checando a carga horaria do prof no semestre 2
              if (professores[p].current_CH2 + turma.workload <= professores[p].maxWorkload2)
              {
                candidate_scores[num_candidates].codigo_turma = p;  // p = 0 --> amaury
                candidate_scores[num_candidates].score        = professores[p].scores[s].score;
                candidate_vars[num_candidates]                = varlist[p * m + turma.label - 1];
      ...
      if (num_candidates > 0)
      {
        ...
        if (turma.semester == 1)
        {
          professores[p].current_CH1 += turma.workload;
          // professores[p].turmasAlocadas[j] = turma.label;
          // professores[p].m++;
        }
        else
        {
          professores[p].current_CH2 += turma.workload;
          // professores[p].turmasAlocadas[j] = turma.label;
          // professores[p].m++;
        }
      }
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      if (professores[i].current_CH1 + professores[i].current_CH2 >= professores[i].minWorkload)
      {
        j++;
      }
    }
    printf("\nNUMERO DE TURMAS COBERTAS AO PERCORRER TODAS AS TURMAS: %d\n", *nCovered);
    printf("NUMERO DE PROFS COM AS HORAS ATINGIDAS: %d\n", j);
...
int grasp(SCIP *scip, SCIP_SOL **sol, SCIP_HEUR *heur)
{
  ...
  n           = I->nProfessors;  // quant de professores
  m           = I->nCourses;     // quant de turmas

  professores = (Professor *) malloc(sizeof(Professor) * n);
  turmas      = (Course *) malloc(sizeof(Course) * m);
  ...
  //      fprintf(arquivo, "codigo_turma: %d\n", professores[i].scores[j].codigo_turma);
  //  }
  // }
  ...
  g_covered = covered;
  qsort(turmas, m, sizeof(Course), compareTurmasByN);
  g_covered                   = NULL;
